...
const MOCK_EVENTS = [
    ...
    },
    {
        ...
        startAt: new Date(new Date().setDate(new Date().getDate() + 6)),
...
const scoreEvent = (event: any, mode: string | null) => {
    ...
    if (mode === 'housewife') {
        ...
        if (event.startAt.getHours() >= 10 && event.startAt.getHours() <= 16) score += 10
    }
    else if (mode === 'worker') {
        if (event.after18) score += 25
        if (event.nearStation) score += 15
        if (event.startAt.getHours() >= 17) score += 15
    }
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        console.error("Database unavailable, falling back to mock data")
    }
